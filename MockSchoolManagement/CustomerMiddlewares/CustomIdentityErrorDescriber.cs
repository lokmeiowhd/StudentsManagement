...
namespace MockSchoolManagement.CustomerMiddlewares
{
    public class CustomIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        {
            return new IdentityError { Code = nameof(InvalidUserName), Description = $"用户名'{userName}'无效，只能包含字母或者数字。" };
        }
        ...
        {
            return new IdentityError { Code = nameof(UserLockoutNotEnabled), Description = "用户未启用锁定。" };
        }
        ...
        public override IdentityError PasswordRequiresNonAlphanumeric()
        {
            return new IdentityError
            {
                ...
                Description = "密码必须至少含有一个非字母数字字符"
            };
        }

        public override IdentityError PasswordRequiresDigit()
        {
            return new IdentityError { Code = nameof(PasswordRequiresDigit), Description = "密码必须至少含有一个数字（0-9）." };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordRequiresLower), Description = "密码必须含有小写字母'a-z'." };
        }

        public override IdentityError PasswordRequiresUpper()
        {
            return new IdentityError { Code = nameof(PasswordRequiresUpper), Description = "密码必须含有大写字母'A-Z'." };
        }
    }
}
